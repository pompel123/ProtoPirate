...
typedef struct SubGhzProtocolDecoderMazda
{
    ...
    uint8_t button;
    uint32_t count;
} SubGhzProtocolDecoderMazda;

typedef struct SubGhzProtocolEncoderMazda
{
    SubGhzProtocolEncoderBase base;
    SubGhzProtocolBlockEncoder encoder;
    SubGhzBlockGeneric generic;
} SubGhzProtocolEncoderMazda;

typedef enum
{
    MazdaDecoderStepReset = 0,
    MazdaDecoderStepPreamble,
    MazdaDecoderStepPreambleCheck,
    MazdaDecoderStepGap,
    MazdaDecoderStepData,
} MazdaDecoderStep;

static void mazda_add_bit(SubGhzProtocolDecoderMazda *instance, bool bit);
static void decode_mazda(uint64_t key1, uint16_t key2, uint32_t *serial, uint8_t *button, uint32_t *count);
static bool mazda_process_data(SubGhzProtocolDecoderMazda *instance);
...
const SubGhzProtocol mazda_protocol = {
...
static bool mazda_process_data(SubGhzProtocolDecoderMazda *instance)
{
    ...
    if (instance->bit_count == 80)
    {
        ...
        decode_mazda(instance->key1, key2, &instance->serial, &instance->button, &instance->count);
        instance->key2 = key2;
        return true;
    }
...
{
    UNUSED(environment);
    SubGhzProtocolDecoderMazda *instance = malloc(sizeof(SubGhzProtocolDecoderMazda));
    instance->base.protocol = &mazda_protocol;
...
void subghz_protocol_decoder_mazda_feed(void *context, bool level, uint32_t duration)
{
    ...
    uint32_t te_short = subghz_protocol_mazda_const.te_short;
    uint32_t te_long = subghz_protocol_mazda_const.te_long;
    uint32_t te_delta = subghz_protocol_mazda_const.te_delta;
    ...
    case MazdaDecoderStepReset:
        if (level && (DURATION_DIFF(duration, te_short) < te_delta))
        {
            instance->data_low = 0;
            instance->data_high = 0;
            instance->decoder.parser_step = MazdaDecoderStepPreamble;
    ...
    case MazdaDecoderStepPreambleCheck:
        if (level)
        {
            ...
            {
                instance->decoder.parser_step = MazdaDecoderStepGap;
            }
            else
            {
                instance->decoder.parser_step = MazdaDecoderStepReset;
    ...
    case MazdaDecoderStepGap:
        if (!level && (DURATION_DIFF(duration, gap_threshold) < 250))
        {
            instance->data_low = 1;
            instance->data_high = 0;
            instance->bit_count = 1;
            instance->decoder.parser_step = MazdaDecoderStepData;
        }
    ...
    case MazdaDecoderStepData:
    {
        ...
        {
            mazda_add_bit(instance, data_bit);

            if (mazda_process_data(instance))
            {
...
SubGhzProtocolStatus subghz_protocol_decoder_mazda_deserialize(void *context, FlipperFormat *flipper_format)
{
    furi_assert(context);
    SubGhzProtocolDecoderMazda *instance = context;
